...
public class EjemploServidorT {
    ...
    private static void cargarDatos() {
        try {
            File archivo = new File("Usuarios_v2.usuarios");
            if (!archivo.exists()) {
                archivo.createNewFile();
            }
            if (archivo.length() > 0) {
                FileInputStream fis = new FileInputStream("Usuarios_v2.usuarios");
    ...
    private static class Handler implements Runnable {
        ...
        private final String PATRON_CONTRA = PATRON_NOMBRE;
        ...
        private final String AYUDA_C = "Comandos:,/nombre_usuario mensaje       Envías mensaje privado,/salir     Sales del Servidor,"
                ...
                + "Muestra tu lista de Usuarios Bloqueados,/cnombre nuevo_nombre       Cambias tu nombre,/ccontra nueva_contra        Cambias tu contra"
                + ",/eusuario       Eliminas tu usuario";
        ...
        private final String CAMBIAR_CONTRA = "ccontra";
        private final String ELIMINAR_USUARIO = "eusuario";
        private final String SEPARADOR = ",";
        private final String INFO_NOMBRE = "El nombre debe estar en Alfanumérico.,No debe estar vacío,No debe tener espacios vacíos,No debe tener más de 15 caracteres, Y no debe ser igual a cualquiera de estos:," + AYUDA + " - " + SALIR + " - " + BLOQUEAR + " - " + DESBLOQUEAR + " - " + MOSTRAR + " - " + MOSTRAR_B + " - " + CAMBIAR_NOMBRE + " - " + CAMBIAR_CONTRA + " - " + ELIMINAR_USUARIO;
        private final String INFO_CONTRA = "La contraseña no debe iniciar igual al nombre,Debe estar en Alfanumérico.,No debe estar vacía,No debe tener espacios vacíos,No debe tener más de 20 caracteres";
        ...
        private void guardarDatos() {
            try {
                fos = new FileOutputStream("Usuarios_v2.usuarios");
        ...
        private int sinRegistrar() {
            ...
            nombre = in.nextLine();
            if (nombre == null || nombre.isEmpty() || nombre.equals("") || !nombre.matches(PATRON_NOMBRE) || nombre.indexOf(' ') >= 0 || nombre.equalsIgnoreCase(AYUDA) || nombre.equalsIgnoreCase(SALIR) || nombre.equalsIgnoreCase(BLOQUEAR) || nombre.equalsIgnoreCase(DESBLOQUEAR) || nombre.equalsIgnoreCase(MOSTRAR) || nombre.equalsIgnoreCase(MOSTRAR_B) || nombre.equalsIgnoreCase(CAMBIAR_NOMBRE) || nombre.equalsIgnoreCase(CAMBIAR_CONTRA) || nombre.equalsIgnoreCase(ELIMINAR_USUARIO) || nombre.startsWith("/") || nombre.length() > 15) {
            ...
            contra = in.nextLine();
            if (contra == null || contra.startsWith(nombre) || contra.isEmpty() || contra.equals("") || !contra.matches(PATRON_CONTRA) || contra.indexOf(' ') >= 0 || contra.startsWith("/") || contra.length() > 20) {
                out.println("INFO2MESSAGE " + INFO_CONTRA);
                return 1;
            }
        ...
        private int Registrado() {
            ...
            if (USUARIOS.containsKey(nombre) && !USUARIOS.get(nombre).getContra().equals(contra)) {
                out.println("INFOMESSAGE La contraseña es incorrecta.");
                return 1;
            }
            return 2;
        }
        ...
        private void cambiarNombre(int espacio, String input) {
            if (espacio >= 0 && espacio < input.length()) {
                ...
                if (nombre2 == null || nombre2.isEmpty() || nombre2.equals("") || nombre2.equals("null") || !nombre2.matches(PATRON_NOMBRE) || nombre2.indexOf(' ') >= 0 || nombre2.equalsIgnoreCase(AYUDA) || nombre2.equalsIgnoreCase(SALIR) || nombre2.equalsIgnoreCase(BLOQUEAR) || nombre2.equalsIgnoreCase(DESBLOQUEAR) || nombre2.equalsIgnoreCase(MOSTRAR) || nombre2.equalsIgnoreCase(MOSTRAR_B) || nombre2.equalsIgnoreCase(CAMBIAR_NOMBRE) || nombre2.equalsIgnoreCase(CAMBIAR_CONTRA) || nombre2.equalsIgnoreCase(ELIMINAR_USUARIO) || nombre2.startsWith("/") || nombre2.length() > 15) {
                    out.println("INFO2MESSAGE " + INFO_NOMBRE);
                } else {
                    if (!nombre.equals(nombre2)) {
        ...
        private void cambiarContra(int espacio, String input) {
            if (espacio >= 0 && espacio < input.length()) {
                String contra2 = input.substring(espacio + 1, input.length());
                if (contra2 == null || contra2.startsWith(nombre) || contra2.equals("null") || contra2.isEmpty() || contra2.equals("") || !contra2.matches(PATRON_CONTRA) || contra2.indexOf(' ') >= 0 || contra2.startsWith("/") || contra2.length() > 20) {
                    out.println("INFO2MESSAGE " + INFO_CONTRA);
                } else {
                    if (!contra.equals(contra2)) {
                        USUARIOS.get(nombre).setContra(contra2);
                        guardarDatos();
                        CONECTADOS.get(nombre).println("INFOMESSAGE Contraseña cambiada");
                        contra = contra2;
                    } else {
                        out.println("INFOMESSAGE Debes poner una contraseña diferente a la que tienes.");
                    }
                }
            } else {
                out.println("INFOMESSAGE Comando no válido, digite '/ayuda' para más información.");
            ...
            try {
                ...
                while (true) {
                    ...
                    synchronized (USUARIOS) {
                        if (!USUARIOS.containsKey(nombre) && !CONECTADOS.containsKey(nombre)) {
                            USUARIOS.put(nombre, new Usuario(contra));
                ...
                while (true) {
                    ...
                    if (input != null && !input.equals("") && !input.isEmpty()) {
                        ...
                        if (input.startsWith("/") && !input2.startsWith("/"+AYUDA) && !input2.startsWith("/"+SALIR) && !input2.startsWith("/"+BLOQUEAR) && !input2.startsWith("/"+DESBLOQUEAR) && !input2.startsWith("/"+MOSTRAR) && !input2.startsWith("/"+MOSTRAR_B) && !input2.startsWith("/"+CAMBIAR_NOMBRE) && !input2.startsWith("/"+CAMBIAR_CONTRA) && !input2.startsWith("/"+ELIMINAR_USUARIO)) {
                            ...
                            } else if (input.toLowerCase().startsWith("/"+CAMBIAR_CONTRA)) {
                                cambiarContra(espacio, input);
