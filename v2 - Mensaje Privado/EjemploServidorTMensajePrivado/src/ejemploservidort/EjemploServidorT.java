...
public class EjemploServidorT {
    ...
    private static Map<String, PrintWriter> USUARIOS = new TreeMap<>();
    ...
    private static class Handler implements Runnable {
        ...
        private final String AYUDA_C = "Comandos:,/nombre_usuario mensaje       Envías mensaje privado,/salir     Sales del Servidor,"
                + "/listau     Muestra la lista de Usuarios,/cnombre nuevo_nombre       Cambias tu nombre";
        private final String SALIR = "salir";
        private final String MOSTRAR = "listau";
        private final String CAMBIAR_NOMBRE = "cnombre";
        private final String SEPARADOR = ",";
        private final String INFO_NOMBRE = "El nombre debe estar en Alfanumérico.,No debe estar vacío,No debe tener espacios vacíos,No debe tener más de 15 caracteres, Y no debe ser igual a cualquiera de estos:," + AYUDA + " - " + SALIR + " - " + MOSTRAR + " - " + CAMBIAR_NOMBRE;
        ...
        private int entrar() {
            ...
            nombre = in.nextLine();
            if (nombre == null || nombre.isEmpty() || nombre.equals("") || !nombre.matches(PATRON_NOMBRE) || nombre.indexOf(' ') >= 0 || nombre.equalsIgnoreCase(AYUDA) || nombre.equalsIgnoreCase(SALIR) || nombre.equalsIgnoreCase(MOSTRAR) || nombre.equalsIgnoreCase(CAMBIAR_NOMBRE) || nombre.startsWith("/") || nombre.length() > 15) {
            ...
            if (espacio >= 0 && espacio < input.length()) {
                ...
                if (USUARIOS.containsKey(nom)) {
                    if (!nombre.equals(nom)) {
                        USUARIOS.get(nombre).println("MESSAGE [MP a " + nom + "] " + nombre + ": " + sub);
                        USUARIOS.get(nom).println("MESSAGE [MP] " + nombre + ": " + sub);
                    } else {
        ...
        private void mensajes(String input) {
            USUARIOS.keySet().forEach((nombre2) -> {
                USUARIOS.get(nombre2).println("MESSAGE " + nombre + ": " + input);
            });
        }
        ...
        private void mostrarUsuarios() {
            ...
            USUARIOS.get(nombre).println("INFO3MESSAGE " + nombres);
            nombres = "";
        }
        ...
        private void cambiarNombre(int espacio, String input) {
            if (espacio >= 0 && espacio < input.length()) {
                ...
                if (nombre2 == null || nombre2.isEmpty() || nombre2.equals("") || !nombre2.matches(PATRON_NOMBRE) || nombre2.indexOf(' ') >= 0 || nombre2.equalsIgnoreCase(AYUDA) || nombre2.equalsIgnoreCase(SALIR) || nombre2.equalsIgnoreCase(MOSTRAR) || nombre2.equalsIgnoreCase(CAMBIAR_NOMBRE) || nombre2.startsWith("/") || nombre2.length() > 15) {
                    out.println("INFO2MESSAGE " + INFO_NOMBRE);
                } else {
                    if (!nombre.equals(nombre2)) {
                        if (!USUARIOS.containsKey(nombre2)) {
                            synchronized (USUARIOS) {
                                PrintWriter pw = USUARIOS.remove(nombre);
                                USUARIOS.put(nombre2, pw);
                                USUARIOS.get(nombre2).println("NAMEACCEPTED " + nombre2);
            ...
            try {
                ...
                while (true) {
                    ...
                    synchronized (USUARIOS) {
                        if (!USUARIOS.containsKey(nombre)) {
                            ...
                            USUARIOS.get(nombre).println("INFOMESSAGE Bienvenido "+nombre);
                            break;
                        }
                    }
                }
                ...
                while (true) {
                    ...
                    if (input != null && !input.equals("") && !input.isEmpty()) {
                        ...
                        if (input.startsWith("/") && !input2.startsWith("/"+AYUDA) && !input2.startsWith("/"+SALIR) && !input2.startsWith("/"+MOSTRAR) && !input2.startsWith("/"+CAMBIAR_NOMBRE)) {
                            mensajePrivado(espacio, input);
                        } else {
                            if (input.toLowerCase().startsWith("/"+SALIR)) {
                                return;
                            } else if (input.toLowerCase().startsWith("/"+AYUDA)) {
                               USUARIOS.get(nombre).println("INFO2MESSAGE "+AYUDA_C);
